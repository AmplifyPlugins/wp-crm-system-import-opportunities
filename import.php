...
if( isset( $_FILES[ 'import_opportunities' ] ) && isset( $_POST[ 'wpcrm_system_import_opportunities_nonce' ] ) ){
	if( wp_verify_nonce( $_POST[ 'wpcrm_system_import_opportunities_nonce' ], 'wpcrm-system-import-opportunities-nonce' ) ){
		...
		if( !in_array( $file_type, $csv_types ) ){
			$errors[] = __( 'File not allowed, please use a CSV file.', 'wp-crm-system-import-opportunities' );
		}
		if( $file_size > return_bytes( ini_get( 'upload_max_filesize' ) ) ){
			$errors[] = __( 'File size must be less than', 'wp-crm-system-import-opportunities' ) . ini_get( 'upload_max_filesize' );
		}				
		if( empty( $errors ) == true ){
					...
					if( null == get_page_by_title( $title, OBJECT, 'wpcrm-opportunity' ) ) {
						...
						if( $opportunityCategories != '' ) {
							...
							if ( is_wp_error( $opportunityTypes ) ) {
								$error[] = _e( 'There was an error with the categories and they could not be set.', 'wp-crm-system-import-opportunities' );
							}
						}
						if( $custom_fields ){
			...
			<div id="message" class="updated">
				<p><strong><?php _e( 'Opportunities uploaded. ', 'wp-crm-system-import-opportunities' ); echo $count_added; _e( ' added. ', 'wp-crm-system-import-opportunities' ); echo $count_skipped; _e( ' skipped.', 'wp-crm-system-import-opportunities' ); ?> </strong></p>
			</div>
		<?php } else { ?>
			...
			<tbody>
				<form id="wpcrm_import_opportunities" name="wpcrm_import_opportunities" method="post" action="" enctype="multipart/form-data">
					<tr>
						...
						<td>
							...
							$link = sprintf( wp_kses( __( 'Important: Please make sure your CSV file is in the <a href="%s">correct format</a>. If it is out of order, your fields will not be imported correctly. ', 'wp-crm-system-import-opportunities' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( $url ) );
							echo $link; ?>
						</td>
					</tr>
				</form>
				<form id="wpcrm_export_opportunities" name="wpcrm_export_opportunities" method="post" action="">
					<tr>
						...
						<input type="submit" name="export_opportunities" value="<?php _e( 'Export Opportunities', 'wp-crm-system-import-opportunities' ); ?>" /></td>
						<td></td>
					</tr>
				</form>
			</tbody>
